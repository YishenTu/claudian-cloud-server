...
async function verifyRuntimeImage(): Promise<void> {
  ...
  try {
    ...
    const vaultCredential = randomBytes(32).toString('hex');
    const authenticated = await fetch(`${origin}${protectedRoute.target}`, {
      body: protectedBody,
      headers: {
        authorization: `Bearer ${vaultCredential}`,
    ...
    const untrusted = await fetch(`${origin}${protectedRoute.target}`, {
      ...
      headers: {
        'content-type': 'application/json',
        'x-claudian-ingress-principal': 'principal-runtime-image',
      },
      method: protectedRoute.method,
    });
    ...
    assert.equal(stdout.join('').includes(vaultCredential), false);
