...
describe('decodeServerConfig', () => {
  it('decodes an immutable private-development configuration', () => {
  ...
  it('rejects a non-loopback private-development bind', () => {
    assert.throws(
      () => decodeServerConfig({
        ...validSource,
  ...
  it('rejects production trusted-ingress settings in private development', () => {
    assert.throws(
      () => decodeServerConfig({
        ...validSource,
  ...
  it('rejects malformed numeric limits', () => {
    for (const [field, value] of [
      ...
      ['CLAUDIAN_CLOUD_SHUTDOWN_TIMEOUT_MS', '99'],
    ] as const) {
      assert.throws(
