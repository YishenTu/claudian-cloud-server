...
The published LAN upgrade and Cloud member-claim gate uses an isolated real server, PostgreSQL and Git with the client checkout's pinned 2.2.6 baseline. From this repository, run `node --import tsx tests/helpers/AuthorityRoundtripGate.ts /absolute/path/to/claudian --published-lan` with the PostgreSQL test environment configured. It verifies existing LAN compatibility, split Host/Manager migration, wrong-target rejection, and recovery after Cloud commits a member claim but its response is lost. The client checkout must include `test:lan-compatibility`; this cross-repository gate is separate from the server-only CI jobs.
