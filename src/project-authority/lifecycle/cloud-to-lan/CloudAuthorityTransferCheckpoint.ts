...
import type { OutboundProjectCheckpointReservation, ProjectCheckpointCoordinator } from '../../checkpoint/ProjectCheckpointCoordinator.js';
import type {
...
implements CloudToLanCheckpointCapturePort {
  ...
  reserve(projectId: string): Promise<OutboundProjectCheckpointReservation> {
    return this.#checkpoint.reserveOutbound(projectId);
  }

  async capture(
    input: Parameters<CloudToLanCheckpointCapturePort['capture']>[0],
    ...
    const snapshot = await input.lease.withProjectScope(async scope => {
      ...
      const records = await scope.checkpoint.readProjectCheckpointRecords({
        ...
        maximumCoordinationBytes: reservation.maximumCoordinationBytes,
        metadata: {
