...
- The checksum-verified serial lane is `0002_development_bootstrap.sql`, `0003_project_read_events.sql`, `0004_collaboration.sql`, `0005_accept_recovery.sql`, `0006_portability_lifecycle.sql`, `0007_lan_to_cloud_transfer.sql`, `0008_cloud_to_lan_transfer.sql`, then `0009_terminal_project_lifecycle.sql`. Each migration change owns its registry entry, grants, forced-RLS policy, and real PostgreSQL evidence.
...
- LAN-to-Cloud checkpoint validation imports exact portable records into canonical Project relations in one transaction at the target generation with `maintenance` service state and no placement, principal binding, or event. Activation promotes that exact inactive state; pre-relinquishment cancellation may remove it only under the exact transfer journal and stage digest.
...
- Terminal responder retention expires on acknowledgement/deadline independently of repository deletion progress. Deletion continuity therefore treats the exact responder as optional after expiry while retaining the tombstone, deletion journal/candidate, authorization, and protected continuity records.
