...
- Empty Cloud Project publication consumes only a Project-scoped reservation acquired before the caller's Project lease and one persisted deterministic plan. It creates or verifies the exact empty tree and initial commit, protected main, initial Manager personal ref, object format, storage identity, and creation marker. A partial exact repository is replayable; a divergent object, ref, marker, path, or placement fails closed and is never overwritten or deleted by creation recovery.
- Join personal-ref creation consumes one opaque Project-scoped repository reservation acquired before the caller's Project lease. It creates or replays only the exact persisted ref at the exact persisted main OID, rejects every unrelated ref change, and never acquires capacity from inside the lease.
...
- Join creates only the persisted Member personal ref at its pinned main OID with missing-ref CAS and exact replay verification. Removal reads and then persists the target ref OID while holding its pre-lease reservation. Removal and Leave delete only that persisted personal ref at its expected OID; a changed ref or any extra effect fails closed.
